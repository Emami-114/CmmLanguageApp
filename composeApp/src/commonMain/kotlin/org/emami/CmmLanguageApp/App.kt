...
import org.emami.CmmLanguageApp.data.repository.WordRepository
...
internal fun App() = AppTheme {
    ...
    var listWord by remember { mutableStateOf(listOf<Word>(Word("casc","ascas"))) }

    LaunchedEffect(Unit) {
        listWord = WordRepository().getWords()
    }

    LazyColumn {
        items(listWord) {
            Text("${it.word} == ${it.meaning}")
        }
    }

//    LoginContent(email, password, passwordVisibility)
}
...
private fun LoginContent(
    ...
    var passwordVisibility1 = passwordVisibility
        ...
        OutlinedTextField(
            ...
            trailingIcon = {
                IconButton(onClick = { passwordVisibility1 = !passwordVisibility1 }) {
                    val imageVector =
                        if (passwordVisibility1) Icons.Default.Close else Icons.Default.Edit
                    Icon(
                        imageVector,
